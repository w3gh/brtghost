...
CBaseGame :: CBaseGame( CGHost *nGHost, CConfigData* nConfig, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nOwnerName, string nCreatorName, string nCreatorServer )
{
	m_GHost = nGHost;
	...
	if( m_Config->issavereplays && !m_SaveGame )
	...
	if( m_Config->m_ReconnectWaitTime == 0 )
		m_GProxyEmptyActions = 0;
	else
	{
		m_GProxyEmptyActions = m_Config->m_ReconnectWaitTime - 1;
...
//	m_ShowRealSlotCount = m_GHost->m_ShowRealSlotCount;
	...
	m_SwitchNr = 0;
	m_Switched = false;
//	m_CreatorAsFriend = m_Config->m_AddCreatorAsFriendOnHost;
	m_GarenaOnly = false;
	m_NoGarena = false;
...
//	m_LocalAdminMessages = m_GHost->m_LocalAdminMessages;
	...
	if( !m_Config->m_BindAddress.empty( ) )
		CONSOLE_Print( "[GAME: " + m_GameName + "] attempting to bind to address [" + m_Config->m_BindAddress + "]" );
	else
		CONSOLE_Print( "[GAME: " + m_GameName + "] attempting to bind to all available addresses" );

	if( m_Socket->Listen( m_Config->m_BindAddress, m_HostPort ) )
	...
	if (m_GetMapType == "dota" && m_Config->m_AutoStartDotaGames)
	...
	if (m_Replay)
	{
		if ( ( m_GameLoading || m_GameLoaded ) )
		{
			...
			boost::thread nReplayThread( SaveReplay, 
										...
										m_StatString,
										m_Config->replaypath, 
										m_Config->m_ReplayTimeShift, 
										m_Config->m_ReplayBuildNumber,
										m_Config->m_ReplayWar3Version,
										m_GameName,
										m_DatabaseID );
		} else
			delete m_Replay;
	...
	for( vector<CCallableWarnCount *> :: iterator i = m_WarnCounts.begin( ); i != m_WarnCounts.end( ); )
	{
		if( (*i)->GetReady( ) )
		{
			...
			if (Player)
			{
				...
				if (Count > 0)
				{
					SendChat( Player, tr("lang_0501", UTIL_ToString(Count) ) ); // DisplayWarnsCount
				}
			}
...
//	if( !m_GameLoading && !m_GameLoaded && GetNumPlayers( ) < 12 )
	...
	{
		SendAllChat( tr("lang_0117") ); // Game unlocked
		m_Locked = false;
	}
	...
	if( GetTime( ) - m_LastPingTime >= 5 )
	{
		...
		if( !m_CountDownStarted )
		{
			...
			if( m_SaveGame )
			{
				...
				BYTEARRAY data = m_Protocol->SEND_W3GS_GAMEINFO( m_Config->tft, 
					m_Config->m_LANWar3Version, 
...
//				uint32_t MapGameType = MAPGAMETYPE_UNKNOWN0;
	...
	if( !m_RefreshError && !m_CountDownStarted && m_GameState == GAME_PUBLIC && GetSlotsOpen( ) > 0 && GetTime( )- m_LastRefreshTime >= 3 )
	{
		...
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			...
			{
				if(m_Config->m_Refresh0Uptime)
					(*i)->QueueGameRefresh( m_GameState, m_GameName, string( ), m_Map.get(), m_SaveGame, 0, m_HostCounter );
				else
		...
		if( m_Config->m_RefreshMessages && Refreshed )
			SendAllChat( tr("lang_0042") ); // Game refreshed
...
*/
	...
	if( !m_CountDownStarted && m_Config->m_RequireSpoofChecks && m_GameState == GAME_PUBLIC && !m_Config->m_AutoHostGameName.empty( ) && m_Config->m_AutoHostMaximumGames && m_Config->m_AutoHostAutoStartPlayers && m_AutoStartPlayers )
	{
		for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
		{
			...
			{
				(*i)->SetDeleteMe( true );
				(*i)->SetLeftReason( tr("lang_0167") ); // WasKickedForNotSpoofChecking
	...
	{
		StartCountDownAuto( m_Config->m_RequireSpoofChecks );
		m_LastAutoStartTime = GetTime( );
	}
	...
	if( m_GameEndCountDownStarted && GetTicks( ) - m_GameEndLastCountDownTicks >= 1000 )
	{
		...
		{
			m_GameEndCountDownStarted = false;
			StopPlayers( tr("lang_1007") ); // was disconnected (admin ended game)
		}
	...
	if (m_Config->m_AutoRehostDelay)
//	if (LoggedIn)
	if (GetTime() > m_CreationTime + 5)
	if (GetTime() > m_LastPlayerJoiningTime + m_Config->m_AutoRehostDelay && !m_GameLoading && !m_GameLoaded && !m_AllSlotsOccupied)
	{
...
//				m_LastReservedSeen = GetTime( );
	...
	if( m_GameLoaded )
	{
		...
		if( !m_Lagging )
		{
			...
			for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
			{
				...
				if( Sync > m_Config->m_SyncLimit )
				{
					(*i)->SetLagging( true );
		// calculate drop vote ticks
					uint32_t d = 5;
					if (m_Config->m_DropVoteTime < 45)
						d = (45 * 1000) - m_Config->m_DropVoteTime * 1000 ;
		...
		if( m_Lagging )
		{
			...
			if( GetTime( ) - m_StartedLaggingTime >= WaitTime )
				StopLaggers( tr("lang_0214", UTIL_ToString( WaitTime ) ) ); // WasAutomaticallyDroppedAfterSeconds( UTIL_ToString( WaitTime ) )
			...
			for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
			{
				if( (*i)->GetLagging( ) && m_SyncCounter - (*i)->GetSyncCounter( ) < m_Config->m_SyncLimit / 2 )
				{
	...
	{
		SendAllChat( tr("lang_1008") ); // End request expired.
		m_EndRequested = false;
		m_RequestedWinner = 0;
		m_EndRequestedTicks = 0;
	}
	...
	if( !m_RmkVotePlayer.empty( ) && GetTime( ) - m_StartedRmkVoteTime >= 180 )
	{
		...
		SendAllChat(tr("lang_1009") );  // Rmk vote expired.
	...
	if( !m_KickVotePlayer.empty( ) && GetTime( ) - m_StartedKickVoteTime >= 60 )
	{
		...
		SendAllChat( tr( "lang_0163", m_KickVotePlayer ) );  // VoteKickExpired( m_KickVotePlayer )
	...
	if (!m_GameOverCanceled)
	{
		...
		if (!m_GameEnded && m_Config->m_gameoverminpercent!=0 && ( !m_GameLoading && m_GameLoaded ) && remainingpercent < m_Config->m_gameoverminpercent)
				lessthanminpercent = true;

		if (lessthanminpercent && m_GameOverTime == 0)
		{
			CONSOLE_Print( "[GAME: " + m_GameName + "] gameover timer started (less than "+UTIL_ToString(m_Config->m_gameoverminpercent)+"% )"+" "+string(1,m_Config->m_CommandTrigger)+"override to cancel" );
			SendAllChat(tr("lang_1010", "$TIME$", UTIL_ToString(remainingpercent), 
																	"$PERCENT$", UTIL_ToString(m_Config->m_gameoverminpercent), 
																	"TRIGGER", string(1, m_Config->m_CommandTrigger) ) );  // "Game over in 60 seconds, "+ UTIL_ToString(remainingpercent)+"% remaining ( < "+UTIL_ToString(m_GHost->m_gameoverminpercent)+"% ) "+string(1,m_Config->m_CommandTrigger)+"override to cancel"
		...
		if (!m_GameEnded && m_Config->m_gameoverminplayers  && m_Players.size( ) < m_Config->m_gameoverminplayers && m_PlayersatStart > m_Players.size() && ( !m_GameLoading && m_GameLoaded ) )
				lessthanminplayers = true;

		if (lessthanminplayers && m_GameOverTime == 0)
		{
			CONSOLE_Print( "[GAME: " + m_GameName + "] gameover timer started (one player left)"+" "+string(1, m_Config->m_CommandTrigger)+"override to cancel" );
			SendAllChat(tr("lang_1011", "$TIME$", UTIL_ToString(m_Players.size()) , 
																  "$COUNT$", UTIL_ToString(m_Config->m_gameoverminplayers), 
																  "$TRIGGER$", string(1,m_Config->m_CommandTrigger)  )  );     // "Game over in 60 seconds, "+ UTIL_ToString(m_Players.size())+" remaining ( < "+UTIL_ToString(m_GHost->m_gameoverminplayers)+" ) "+string(1, m_Config->m_CommandTrigger)+"override to cancel"
			m_GameOverTime = GetTime( );
		}
		...
		if (!lessthanminplayers && !lessthanminpercent)
		if (m_GetMapNumTeams==2 && m_Config->m_gameovermaxteamdifference && m_Players.size() < m_PlayersatStart && m_Players.size()>1)
		{
			bool unbalanced = false;

			if (m_TeamDiff > m_Config->m_gameovermaxteamdifference)
				unbalanced = true;
			...
			if (m_GameOverTime!=0 && !unbalanced)
			{
				...
				SendAllChat( tr("lang_1012")); // "Game over averted, team rebalanced"
			...
			if (unbalanced && m_GameOverTime==0)
			{
				...
				SendAllChat( tr("lang_1013", UTIL_ToString(m_Config->m_gameovermaxteamdifference), string(1, m_Config->m_CommandTrigger)));  // "Game over in 120 seconds, rebalance quickly!, max team difference is "+UTIL_ToString(m_GHost->m_gameovermaxteamdifference)+" "+string(1, m_Config->m_CommandTrigger)+"override to cancel"
		...
		if( m_GameOverTime != 0 && GetTime( ) >= m_GameOverTime + 60 )
		{
			...
			if( !AlreadyStopped )
			{
				...
				StopPlayers( tr("lang_1014") );    // was disconnected (gameover timer finished)
			}
		}
	}
	...
	//check if we have hit the timelimit
		if ( GetTime( ) > m_LastReservedSeen + m_Config->m_LobbyTimeLimit*60 )
		{
			...
			for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
			{
                ...
                (*i)->QueueChatCommand( tr("lang_1015", m_GameName) ); // "[GAME: " + m_GameName + "] is over (lobby timelimit hit)"

				if( (*i)->GetServer( ) == GetCreatorServer( ) )
					(*i)->QueueChatCommand( tr("lang_1015", m_GameName), GetCreatorName( ), true );    // "[GAME: " + m_GameName + "] is over (lobby timelimit hit)"
			}
		m_Exiting = true;
		}
	}
	...
	if (GetTime()-m_SlotsOccupiedTime>3)
	{
		...
		for( vector<CGamePlayer *> :: iterator i = SortedPlayers.begin( ); i != SortedPlayers.end( ); i++ )
		{
			...
			if( (*i)->GetNumPings( ) > 0 )
			{
				Ping=(*i)->GetPing( m_Config->m_LCPings );
				if (Ping>5)
				{
					skipP = false;
...
{
	if( !m_Config->m_LocalAdminMessages )
		return;
...
void CBaseGame :: SendVirtualHostPlayerInfo( CGamePlayer *player )
{
	...
	IP.push_back( 0 );
	IP.push_back( 0 );
	Send( player, m_Protocol->SEND_W3GS_PLAYERINFO( m_VirtualHostPID, m_Config->m_VirtualHostName, IP, IP ) );
}
...
//	SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
	if (m_CountryCheck || m_CountryCheck2 || m_ProviderCheck || m_ProviderCheck2)
	{
		...
		if (m_CountryCheck)
			SendChat( player, tr("lang_1020",C1) ); // SendChat( player, "Allowed Countries = "+C1 );
		if (m_CountryCheck2 && !m_CountryCheck)
			SendChat( player, tr("lang_1021",C2) );   // "Denied Countries = "+C2
		if (m_ProviderCheck)
			SendChat( player, tr("lang_1022",m_Providers) ); // "Allowed Providers = "+m_Providers
		if (m_ProviderCheck2)
			SendChat( player, tr("lang_1023",m_Providers2) ); // "Denied Providers = "+m_Providers2
	}
//	SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
//	SendChat( player, "Owner: "+m_OwnerName+"    Game Name:     " + m_GameName );
	if( !m_HCLCommandString.empty( ) )
        SendChat( player, tr("lang_1024", m_HCLCommandString));     // "     HCL Command String:  " + m_HCLCommandString
...
void CBaseGame :: EventPlayerDeleted( CGamePlayer *player )
{
	...
	if( player->GetWhoisSent( ) && !player->GetJoinedRealm( ).empty( ) && player->GetSpoofedRealm( ).empty( ) )
	{
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			if( (*i)->GetServer( ) == player->GetJoinedRealm( ) )
			{
				...
				(*i)->UnqueueChatCommand( "/w " + player->GetName( ) + " " + tr("lang_0041") ); // SpoofCheckByReplying
			}
		}
	}
	...
	if( m_GameLoaded && player->GetLeftCode( ) == PLAYERLEAVE_DISCONNECT && m_Config->m_AutoSave )
	{
	...
	if( !m_RmkVotePlayer.empty( ) )
		SendAllChat( tr("lang_1025") );    // "Rmk vote canceled."
...
void CBaseGame :: EventPlayerDisconnectTimedOut( CGamePlayer *player )
{
	if( player->GetGProxy( ) && m_GameLoaded )
	{
		...
		{
			SendAllChat( player->GetName( ) + " " + tr("lang_0215") );  // HasLostConnectionTimedOutGProxy( )
		...
		if( GetTime( ) - player->GetLastGProxyWaitNoticeSentTime( ) >= 20 )
		{
			...
			SendAllChat( player->GetPID( ), tr("lang_0218", UTIL_ToString( TimeRemaining ) ) );  //  WaitForReconnectSecondsRemain( UTIL_ToString( TimeRemaining ) )
	...
	if( GetTime( ) - m_LastLagScreenTime >= 10 )
	{
		...
		player->SetLeftReason( tr("lang_0084") ); // HasLostConnectionTimedOut( )
...
void CBaseGame :: EventPlayerDisconnectPlayerError( CGamePlayer *player )
{
	...
	player->SetDeleteMe( true );
	player->SetLeftReason( tr("lang_0079", player->GetErrorString( ))); // HasLostConnectionPlayerError( player->GetErrorString( ) )
	player->SetLeftCode( PLAYERLEAVE_DISCONNECT );
...
void CBaseGame :: EventPlayerDisconnectSocketError( CGamePlayer *player )
{
	if( player->GetGProxy( ) && m_GameLoaded )
	{
		if( !player->GetGProxyDisconnectNoticeSent( ) )
		{
			SendAllChat( player->GetName( ) + " " + tr("lang_0216", player->GetSocket( )->GetErrorString( ))  );  // HasLostConnectionSocketErrorGProxy( player->GetSocket( )->GetErrorString( ) )
			player->SetGProxyDisconnectNoticeSent( true );
		}
	...
	player->SetDeleteMe( true );
	player->SetLeftReason( tr("lang_0080", player->GetSocket( )->GetErrorString( ) )); //     HasLostConnectionSocketError( player->GetSocket( )->GetErrorString( ) )
	player->SetLeftCode( PLAYERLEAVE_DISCONNECT );
...
void CBaseGame :: EventPlayerDisconnectConnectionClosed( CGamePlayer *player )
{
	if( player->GetGProxy( ) && m_GameLoaded )
	{
		if( !player->GetGProxyDisconnectNoticeSent( ) )
		{
			SendAllChat( player->GetName( ) + " " + tr("lang_0217")  ); //  HasLostConnectionClosedByRemoteHostGProxy( )
			player->SetGProxyDisconnectNoticeSent( true );
		}
	...
	player->SetDeleteMe( true );
	player->SetLeftReason( tr("lang_0081") );// HasLostConnectionClosedByRemoteHost
	player->SetLeftCode( PLAYERLEAVE_DISCONNECT ); 
...
void CBaseGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer )
{
	...
	if( joinPlayer->GetName( ) == m_Config->m_VirtualHostName )
	{
						...
						if (!bad)
							...
							if (!bad)
								...
								if (bad)
								{
									...
									if (m_Config->m_Verbose && m_Config->m_ShowCountryNotAllowed)
...
// check if we're only allowing certain scores
	...
	if (m_ScoreCheckChecked)
	{
		...
		bool allow = false;
		allow = (m_Config->m_AllowNullScoredPlayers && (Score==0));
		if (!allow)
		if (Score < m_Scores)
		{
			string n = joinPlayer->GetName();
	...
	else
	{
		...
		if( SID == 255 && Reserved )
		{
			...
			if( SID != 255 )
			{
				...
				if( KickedPlayer )
				{
					...
					KickedPlayer->SetLeftReason( tr("lang_0076", joinPlayer->GetName( )) );   // WasKickedForReservedPlayer( joinPlayer->GetName( ) )
		...
		if( SID == 255 && IsOwner( joinPlayer->GetName( ) ) )
		{
			...
			{
				KickedPlayer->SetDeleteMe( true );
				KickedPlayer->SetLeftReason( tr("lang_0077", joinPlayer->GetName( ) ) );  // WasKickedForOwnerPlayer( joinPlayer->GetName( ) )
				KickedPlayer->SetLeftCode( PLAYERLEAVE_LOBBY );
	...
	if( SID >= m_Slots.size( ) )
	{
		...
		{
			m_LastAdminJoinAndFullTicks = GetTicks();
			SendAllChat(tr("lang_1028",joinPlayer->GetName()));  // "Admin "+joinPlayer->GetName()+" is trying to join but the lobby is full..."
		}
		potential->Send( m_Protocol->SEND_W3GS_REJECTJOIN( REJECTJOIN_FULL ) );
		potential->SetDeleteMe( true );
		return;
	...
	if ( m_Config->m_Banning && !AdminCheck && !m_ScoreCheckChecked )
	for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
	{
		if ( JoinedRealm == (*i)->GetServer( ) ) 
		{
			...
			if( Ban )
			{
				...
				*/
				...
				if ( m_Config->m_Verbose && m_AnnouncedPlayer != joinPlayer->GetName() )
				{
					...
					if (m_Config->m_Banning == 2)
					if (sReason.empty())
						SendAllChat( sBan );
					else
					{
						if (sBReason.length() < 250 && !m_Config->m_TwoLinesBanAnnouncement)
...
*/
	...
	// check if the new player's ip is banned
		if ( !m_ScoreCheckChecked && !potential->IsLAN() && m_Config->m_IPBanning)
			for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
			{
				if ( JoinedRealm != (*i)->GetServer( ) ) 
					continue;
...
//								SendAllChat ( sName +"("+potential->GetExternalIPString()+")");
					...
					if ( m_Config->m_IPBanning == 1 && m_Bans )
					{
						// let banned players "join" the game with an arbitrary PID then immediately close the connection
						// this causes them to be kicked back to the chat channel on battle.net
...
*/
					...
					if ( m_Config->m_IPBanning == 2 )
					{
						string sBan = tr("lang_1026", "$JOINPLAYER$", joinPlayer->GetName(), "$BANNEDPLAYER$", sName, "$ADMIN$", sAdmin);   //joinPlayer->GetName()+"(IP banned as "+sName+") by "+sAdmin;
//						string sBan = joinPlayer->GetName()+"("+potential->GetExternalIPString()+") is IP banned by "+sAdmin;
						...
						{
							if (sBReason.length() < 220 && !m_Config->m_TwoLinesBanAnnouncement)
								SendAllChat( sBReason );
							else
							{
								SendAllChat( sBan );
								SendAllChat( tr("lang_1027", sReason)  );   // "Ban reason: " + sReason
							}
						}
					}
				}
...
//			SendAllSlotInfo( );
	...
	bool dontplacehigher = m_Config->m_PlaceAdminsHigherOnlyInDota && (m_GetMapType != "dota");
...
//	CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + joinPlayer->GetName( ) + "|" + potential->GetExternalIPString( ) + "] joined the game" );
	...
	if ( !m_Config->m_RootAdminsSpoofCheck && RootAdminCheck)
	{
		Player->SetSpoofed( true );
		Player->SetSpoofedRealm(m_CreatorServer);
	}

	if ( !m_Config->m_AdminsSpoofCheck && AdminCheck)
	{
		Player->SetSpoofed( true );
		Player->SetSpoofedRealm(m_CreatorServer);
	}
...
//	if( UTIL_IsLanIP( Player->GetExternalIP( ) ) || UTIL_IsLocalIP( Player->GetExternalIP( ), m_GHost->m_LocalAddresses ) )
	...
	Player->SetWhoisShouldBeSent( m_Config->m_SpoofChecks == 1 || ( m_Config->m_SpoofChecks == 2 && AnyAdminCheck ) );
	...
	CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + Player->GetName( ) + "] [" + Player->GetExternalIPString() + "] joined the game" );
...
// recalculate nr of players in each team + difference in players nr.
	...
	if ( m_Config->m_CheckMultipleIPUsage )
	{
		...
		if (IPs.size() > 0 && sayit)
		{
			string IPString;
			p = Players.begin();
			IPString = tr("lang_1029"); //"Same IPs: ";
			for (vector<string> :: iterator s = IPs.begin(); s != IPs.end(); s++)
			{
				IPString += (*p);
				if (s!=IPs.end()-1)
	...
	if( m_Config->m_RequireSpoofChecks && !Player->GetWhoisShouldBeSent( ) )
	{
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			...
			if( (*i)->GetServer( ) == JoinedRealm )
				SendChat( Player, tr( "lang_0067", string( UniqueName.begin( ), UniqueName.end( ) )  ) ); // SpoofCheckByWhispering
		}
	}
	...
	if (m_Config->m_ShowScoresOnJoin)
		m_ShowScoreOf = Player->GetName();

	// show current player's note
	if (m_Config->m_ShowNotesOnJoin)
...
//			SendAllChat( m_GHost->m_Language->MultipleIPAddressUsageDetected( joinPlayer->GetName( ), Others ) );
	...
	if( m_Config->m_AutoLock && !m_Locked && IsOwner( joinPlayer->GetName( ) ) )
	{
...
void CBaseGame :: EventPlayerJoinedWithScore( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer, double score )
{
	...
	if( SID == 255 )
	{
		...
		if( m_Config->m_MatchMakingMethod == 0 )
		{
			// method 0: don't do any matchmaking
			// that was easy!
		}
		else if( m_Config->m_MatchMakingMethod == 1 )
		{
			...
			if( FurthestPlayer->GetScore( ) < -99999.0 )
				FurthestPlayer->SetLeftReason( tr("lang_0168", "$SCORE$", tr("lang_1018"), "$AVERAGE$", UTIL_ToString( AverageScore, 2 )) );    // WasKickedForHavingFurthestScore( "N/A", UTIL_ToString( AverageScore, 2 )
			else
                FurthestPlayer->SetLeftReason( tr("lang_0168", "$SCORE$", UTIL_ToString( FurthestPlayer->GetScore( ), 2 ), "$AVERAGE$", UTIL_ToString( AverageScore, 2 )) ); // WasKickedForHavingFurthestScore( UTIL_ToString( FurthestPlayer->GetScore( ), 2 ), UTIL_ToString( AverageScore, 2 ) )
			...
			if( FurthestPlayer->GetScore( ) < -99999.0 )
				SendAllChat( tr("lang_0199", "$SCORE$", tr("lang_1018"), "$AVERAGE$", UTIL_ToString( AverageScore, 2 ))); //  PlayerWasKickedForFurthestScore( FurthestPlayer->GetName( ), "N/A", UTIL_ToString( AverageScore, 2 ) )
			else
				SendAllChat( tr("lang_0199", "$SCORE$", UTIL_ToString( FurthestPlayer->GetScore( ), 2 ), "$AVERAGE$", UTIL_ToString( AverageScore, 2 ))  ); //  PlayerWasKickedForFurthestScore( FurthestPlayer->GetName( ), UTIL_ToString( FurthestPlayer->GetScore( ), 2 ), UTIL_ToString( AverageScore, 2 ) )
		}
		else if( m_Config->m_MatchMakingMethod == 2 )
		{
			...
			if( LowestPlayer->GetScore( ) < -99999.0 )
				LowestPlayer->SetLeftReason( tr("lang_0203", tr("lang_1018") ) );  // WasKickedForHavingLowestScore( "N/A" )
			else
				LowestPlayer->SetLeftReason( tr("lang_0203", UTIL_ToString( LowestPlayer->GetScore( ), 2 ) ) ); // WasKickedForHavingLowestScore( UTIL_ToString( LowestPlayer->GetScore( ), 2 ) )
			...
			if( LowestPlayer->GetScore( ) < -99999.0 )
				SendAllChat( tr("lang_0204", "$NAME$", LowestPlayer->GetName( ), "$SCORE$", tr("lang_1018")  ) ); // PlayerWasKickedForLowestScore( LowestPlayer->GetName( ), "N/A" )
			else
				SendAllChat( tr("lang_0204", "$NAME$", LowestPlayer->GetName( ), "$SCORE$", UTIL_ToString( LowestPlayer->GetScore( ), 2 )  ) ); //lang_0204 PlayerWasKickedForLowestScore( LowestPlayer->GetName( ), UTIL_ToString( LowestPlayer->GetScore( ), 2 ) )
		}
	}
...
/*
			...
			string s(16-n.length(),' ');
			n=s+n;*/
			if (m_Config->m_IPBanning == 1 && m_Config->m_Verbose)
			{
...
//					SendAllChat ( sName +"("+Player->GetExternalIPString()+") ban reason: "+sReason);
			...
			if (m_Config->m_IPBanning == 1)
			{
				Player->SetDeleteMe( true );
				Player->SetLeftReason( tr("lang_1030", "$NAME$", Player->GetName( ), "$IP$", Player->GetExternalIPString()));   //  "was autokicked, " + Player->GetName( ) +Player->GetExternalIPString() + " IP not allowed"
				OpenSlot( GetSIDFromPID( Player->GetPID( ) ), false );
				m_Players.pop_back();
				return;
			}
			string sBan = tr("lang_1031", "$NAME$", Player->GetName(), "$IP$", Player->GetExternalIPString()); //			string sBan = Player->GetName()+"("+Player->GetExternalIPString()+") is IP banned";
			string sBReason = sBan + ", "+sReason;
...
// kick if not garena, admin, rootadmin, reserver
	...
	if (m_CountryCheck || m_CountryCheck2 || m_ProviderCheck || m_ProviderCheck2)
	{
		...
		//			potential->SetDeleteMe( true );
				Player->SetDeleteMe( true );
				Player->SetLeftReason( tr("lang_1033", From)); // "was autokicked, " + From + " not allowed"
				Player->SetLeftCode( PLAYERLEAVE_LOBBY );
...
// recalculate nr of players in each team + difference in players nr.
	...
	if( score < -99999.0 )
		SendAllChat( tr("lang_0169", "$PLAYER$", joinPlayer->GetName( ), "$SCORE$", tr("lang_1018") )); // PlayerHasScore( joinPlayer->GetName( ), "N/A" )
	else
		SendAllChat(  tr("lang_0169","$PLAYER$", joinPlayer->GetName( ), "$SCORE$", UTIL_ToString( score, 2 ) ) ); // PlayerHasScore( joinPlayer->GetName( ), UTIL_ToString( score, 2 ) )
	...
	SendAllChat( tr("lang_0170", "$RATED$", UTIL_ToString( PlayersScored ), "$TOTAL$", UTIL_ToString( PlayersScored + PlayersNotScored ), "$SPREAD$", UTIL_ToString( (uint32_t)Spread ) )); // RatedPlayersSpread( UTIL_ToString( PlayersScored ), UTIL_ToString( PlayersScored + PlayersNotScored ), UTIL_ToString( (uint32_t)Spread ) )
...
//			SendAllChat( m_GHost->m_Language->MultipleIPAddressUsageDetected( joinPlayer->GetName( ), Others ) );
	...
	if( m_Config->m_AutoLock && !m_Locked && IsOwner( joinPlayer->GetName( ) ) )
	{
		SendAllChat( tr("lang_0116") ); // Game Locked
		m_Locked = true;
	}
...
void CBaseGame :: EventPlayerLeft( CGamePlayer *player, uint32_t reason )
{
	...
	if( reason == PLAYERLEAVE_GPROXY )
		player->SetLeftReason( tr("lang_0219") ); // WasUnrecoverablyDroppedFromGProxy
	else
		player->SetLeftReason( tr("lang_0082") ); // HasLeftVoluntarily
...
void CBaseGame :: EventPlayerLoaded( CGamePlayer *player )
{
	...
	if( m_LoadInGame )
	{
		...
		{
			if( *i != player && (*i)->GetFinishedLoading( ) )
				SendChat( *i, tr("lang_0177", player->GetName( ) )); // PlayerFinishedLoading( player->GetName( ) )
		}

		if( !FinishedLoading )
			SendChat( player, tr("lang_0178") );  // PleaseWaitPlayersStillLoading( )
	...
	if( m_Config->m_dropifdesync)
	{
		...
		for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
		{
			if( !(*i)->GetDeleteMe( ) && (*i)->GetCheckSums( )->front( ) != FirstCheckSum )
			{
				...
				SendAllChat( tr("lang_0144") );  // DesyncDetected( )
				...
				for( map<uint32_t, vector<unsigned char> > :: iterator j = Bins.begin( ); j != Bins.end( ); j++ )
				{
					...
					SendAllChat( tr("lang_0195", "$NUMBER$", UTIL_ToString( StateNumber ), "$PLAYERS$", Players) ); // PlayersInGameState( UTIL_ToString( StateNumber ), Players )
					StateNumber++;
				}
				...
				if( Tied )
				{
					...
					StopPlayers( tr("lang_0202")); // WasDroppedDesync( )
					AddToReplay = false;
				}
				else
				{
					...
					for( map<uint32_t, vector<unsigned char> > :: iterator j = Bins.begin( ); j != Bins.end( ); j++ )
					{
						...
						if( (*j).first != (*LargestBin).first )
						{
							for( vector<unsigned char> :: iterator k = (*j).second.begin( ); k != (*j).second.end( ); k++ )
							{
								...
								{
									Player->SetDeleteMe( true );
									Player->SetLeftReason( tr("lang_0202") ); // WasDroppedDesync( )
									Player->SetLeftCode( PLAYERLEAVE_LOST );
								}
							}
						}
...
void CBaseGame :: EventPlayerChatToHost( CGamePlayer *player, CIncomingChatPlayer *chatPlayer )
{
	if( chatPlayer->GetFromPID( ) == player->GetPID( ) )
	{
		if( chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGE || chatPlayer->GetType( ) == CIncomingChatPlayer :: CTH_MESSAGEEXTRA )
		{
			...
			bool iscmd = false;
			if( !msg.empty( ) && msg[0] == m_Config->m_CommandTrigger )
				iscmd = true;
			bool Relay = true;
			bool cRelay = !(iscmd && !m_Config->m_RelayChatCommands);
...
//											SendChat(ListenUser, player->GetName( )+": "+chatPlayer->GetMessage( ));
			...
			if( Message == "?trigger" )
				SendChat( player, tr("lang_0211", string( 1, m_Config->m_CommandTrigger )) ); // lang_0211 CommandTrigger(string( 1, m_Config->m_CommandTrigger )))  )
			else if( !Message.empty( ) && Message[0] == m_Config->m_CommandTrigger )
			{
...
//					msg = m_GHost->CensorRemoveDots(msg);
					...
					if (m_Config->m_autoinsultlobby  && msg2 != msg && m_Config->m_CensorMute && !m_GameLoaded && !m_GameLoading)
					{
						...
						{
							string srv = m_Server;
							string nam1 = m_Config->m_VirtualHostName;
							...
							if (safe2)
							{
								...
								if (!m_Config->m_VirtualHostName.empty())
									randoms.push_back(m_Config->m_VirtualHostName);
								for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
								{
					...
					if  (msg2!=msg && m_Config->m_CensorMute && m_GameLoaded)
					{
						bool ok = true;
						if (isAdmin && !m_Config->m_CensorMuteAdmins)
							ok = false;
...
void CBaseGame :: EventPlayerDropRequest( CGamePlayer *player )
{
	...
	uint32_t TimeRemaining = m_Config->m_DropVoteTime - TimePassed;
	if (m_Lagging)
	if (TimePassed < m_Config->m_DropVoteTime)
	{
		SendAllChat(tr("lang_1035", UTIL_ToString(TimeRemaining))); // UTIL_ToString(TimeRemaining)+" seconds grace period remaining"
		player->SetDropVote(false);
	}
	else
	{
		CONSOLE_Print( "[GAME: " + m_GameName + "] player [" + player->GetName( ) + "] voted to drop laggers" );
		SendAllChat( tr("lang_0096", player->GetName( ) ));  //  PlayerVotedToDropLaggers( player->GetName( ) )
		...
		if( (float)Votes / m_Players.size( ) > 0.49 )
			StopLaggers( tr("lang_0095") ); // LaggedOutDroppedByVote( )
	}
}
...
void CBaseGame :: EventPlayerMapSize( CGamePlayer *player, CIncomingMapSize *mapSize )
{
	...
	if( mapSize->GetSizeFlag( ) != 1 || mapSize->GetMapSize( ) != MapSize )
	{
		...
		if( m_Config->m_AllowDownloads != 0 || candownload )
		{
			...
			if( !MapData->empty( ) )
			{
				if( candownload || m_Config->m_AllowDownloads == 1 || ( m_Config->m_AllowDownloads == 2 && player->GetDownloadAllowed( ) ) )
				{
					...
					else
					{
						...
						if (m_Config->m_maxdownloaders)
							if (downloaders > m_Config->m_maxdownloaders)
								downloaders = m_Config->m_maxdownloaders;

						// calculate shared rate in KB/s
						float sharedRate = 1024*10;
						if (m_Config->m_totaldownloadspeed)
							sharedRate = (float) m_Config->m_totaldownloadspeed / (float) downloaders;
						...
						uint32_t toSend = 1;
						float setRate = (float)m_Config->m_clientdownloadspeed;
						bool queued = false;
						...
						if (m_Config->m_maxdownloaders)
							if (DownloadNr > m_Config->m_maxdownloaders)
								queued = true;
						...
						sDownloadInfo = UTIL_ToString(iRate)+" "+tr("lang_1036")+" ("+UTIL_ToString(Percent)+"% "; // lang_1036
						if (!queued && ETA!=0)
							sDownloadInfo += tr("lang_1037", UTIL_ToString(ETA));
						if (queued)
							sDownloadInfo += tr("lang_1038");
						sDownloadInfo += ")";
			...
			{
				player->SetDeleteMe( true );
				player->SetLeftReason( tr("lang_1039") ); // "doesn't have the map and there is no local copy of the map to send"
				player->SetLeftCode( PLAYERLEAVE_LOBBY );
				OpenSlot( GetSIDFromPID( player->GetPID( ) ), false );
			}
		}
	...
	else
	{
		if( player->GetDownloadStarted( ) )
		{
			...
			SendAllChat( tr("lang_0112", "$USER$", player->GetName( ), "$SECONDS$", UTIL_ToString( Seconds, 1 ), "$RATE$", UTIL_ToString( Rate, 1 ))); // PlayerDownloadedTheMap( player->GetName( ), UTIL_ToString( Seconds, 1 ), UTIL_ToString( Rate, 1 ) )
			player->SetDownloadFinished( true );
			player->SetFinishedDownloadingTime( GetTime( ) );

			if (m_DownloadOnlyMode)
			SendChat (player->GetPID(),tr("lang_1041"));
		...
		{
			player->SetDeleteMe( true );
			player->SetLeftReason( tr("lang_1042") ); // does have the map and this is a download only game
			player->SetLeftCode( PLAYERLEAVE_LOBBY );
			OpenSlot( GetSIDFromPID( player->GetPID( ) ), false );
		}
	}
...
void CBaseGame :: EventPlayerPongToHost( CGamePlayer *player, uint32_t pong )
{
	...
	if( !m_GameLoading && !m_GameLoaded && !player->GetDeleteMe( ) && !player->GetReserved( ) && player->GetNumPings( ) >= 3 && player->GetPing( m_Config->m_LCPings ) > m_Config->m_AutoKickPing )
	...
	if(!(m_AllSlotsOccupied && player->GetPID()==m_LastPlayerJoined && GetTime()-m_LastPlayerJoinedTime>6))
	{
		...
		SendAllChat( tr("lang_0063", "$VICTIM$", player->GetName( ), "$PING$", UTIL_ToString( player->GetPing( m_Config->m_LCPings ) ) )); //  AutokickingPlayerForExcessivePing( player->GetName( ), UTIL_ToString( player->GetPing( m_GHost->m_LCPings ) ) )
		player->SetDeleteMe( true );
		player->SetLeftReason( tr("lang_0063", "$VICTIM$", player->GetName( ), "$PING$", UTIL_ToString( player->GetPing( m_Config->m_LCPings ) ) ) ); // "was autokicked for excessive ping of " + UTIL_ToString( player->GetPing( m_GHost->m_LCPings ) )
		player->SetLeftCode( PLAYERLEAVE_LOBBY );
		OpenSlot( GetSIDFromPID( player->GetPID( ) ), false );
	}
}
...
void CBaseGame :: EventGameStarted( )
{
	...
	if (m_Config->m_ShuffleSlotsOnStart)
		ShuffleSlots();
...
void CBaseGame :: EventGameLoaded( )
{
	...
	{
		string Modes =  tr("lang_1044", p->GetName()); // p->GetName()+ "!! You're on slot 1. Enter any map modes now!";
	...
	if( Shortest && Longest )
	{
		SendAllChat( tr("lang_0071", "$USER$", Shortest->GetName( ), "$LOADINGTIME$", UTIL_ToString( (float)( Shortest->GetFinishedLoadingTicks( ) - m_StartedLoadingTicks ) / 1000, 2 ) ) ); // ShortestLoadByPlayer
		SendAllChat( tr("lang_0072", "$USER$", Longest->GetName( ), "$LOADINGTIME$", UTIL_ToString( (float)( Longest->GetFinishedLoadingTicks( ) - m_StartedLoadingTicks ) / 1000, 2 ) ) ); // LongestLoadByPlayer
	}

	for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
		SendChat( *i, tr("lang_0073", UTIL_ToString( (float)( (*i)->GetFinishedLoadingTicks( ) - m_StartedLoadingTicks ) / 1000, 2 ) ) ); // YourLoadingTimeWas
}
...
//					SendAllChat( (*i)->GetName() + " score " + UTIL_ToString((*i)->GetScore()) );
	...
	SendAllChat( tr("lang_0198") ); // BalancingSlotsCompleted
	SendAllSlotInfo( );
	...
	for( unsigned char i = 0; i < 12; i++ )
	{
		...
		if( TeamHasPlayers )
			SendAllChat( tr("lang_0197", "$TEAM$", UTIL_ToString( i + 1 ), "$SCORE$", UTIL_ToString( TeamScore, 2 ) ) ); // TeamCombinedScore
	}
...
void CBaseGame :: AddToSpoofed( string server, string name, bool sendMessage )
{
	...
	if( Player )
	{
		...
		if( sendMessage )
			SendAllChat( tr("lang_0064", "$SERVER$", server, "$USER$", name ) ); // SpoofCheckAcceptedFor
	}
}
...
void CBaseGame :: AddToCensorMuted( string name )
{
	...
	if (Player->GetCensors() == 1)
		secs = m_Config->m_CensorMuteFirstSeconds;
	else if (Player->GetCensors() == 2)
		secs = m_Config->m_CensorMuteSecondSeconds;
	else if (Player->GetCensors() > 2)
		secs = m_Config->m_CensorMuteExcessiveSeconds;

	m_CensorMutedSeconds.push_back( secs );
	m_CensorMutedTime.push_back( GetTime() );

	AddToMuted(name);
	SendAllChat(tr("lang_0989", "$USER$", name, "$TIME$", UTIL_ToString(secs))); // PlayerMutedForBeingFoulMouthed
}
...
void CBaseGame :: ProcessCensorMuted()
{
	if (GetTime() - m_CensorMutedLastTime > 5)
	{
		...
		for (uint32_t i = 0; i<m_CensorMuted.size(); i++)
		{
			...
			{
				SendAllChat(tr("lang_0996", m_CensorMuted[i])); // RemovedPlayerFromTheMuteList
...
void CBaseGame :: AutoSetHCL ( )
{
	...
	bool forceindota = (m_Config->m_forceautohclindota && m_Map->GetMapType().find("dota")!= string ::npos);

	if ((m_Config->m_AutoHclFromGameName || forceindota) && !m_Map->GetMapDefaultHCL().empty())
					...
					if( m_Mode.size( ) <= m_Slots.size( ) )
					{
						...
						if( m_Mode.find_first_not_of( HCLChars ) == string :: npos )
						{
							...
							if (m_Mode != m_HCLCommandString)
								SendAllChat( tr("lang_0182", GetHCL() ) ); // SettingHCL
						}
						else
...
void CBaseGame :: StartCountDown( bool force )
{
	if( !m_CountDownStarted )
	{
		...
		else
		{
			...
			if( m_HCLCommandString.size( ) > GetSlotsOccupied( ) )
			{
				SendAllChat( tr("lang_0186") ); // TheHCLIsTooLongUseForceToStart
				return;
			}
			...
			if( !StillDownloading.empty( ) )
				SendAllChat( tr("lang_0089", StillDownloading ) ); // PlayersStillDownloading

			// check if everyone is spoof checked

			string NotSpoofChecked;

			if( m_Config->m_RequireSpoofChecks )
			{
				for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
				{
					if( !(*i)->GetSpoofed( ) )
				...
				if( !NotSpoofChecked.empty( ) )
					SendAllChat( tr("lang_0065", NotSpoofChecked ) ); // PlayersNotYetSpoofChecked
			}
...
void CBaseGame :: CreateVirtualHost( )
{
	...
	IP.push_back( 0 );
	IP.push_back( 0 );
	SendAll( m_Protocol->SEND_W3GS_PLAYERINFO( m_VirtualHostPID, m_Config->m_VirtualHostName, IP, IP ) );
}
...
string CBaseGame :: CustomReason ( uint32_t ctime, string reason, string name)
{
		...
		|| Reason == "lag" || Reason == "feeder" || Reason == "lagger" || Reason == "idiot" || Reason == "f" || Reason == "n" || Reason == "mh" || Reason == "m")
	{
		...
		Reason = tr("lang_0994", "$REASON$", Reason, 
															"$TIME$", sTime, 
															"$DATE$", sDate, 
															"$TEAMS$", sTeam, 
...
void CBaseGame :: SetDynamicLatency( )
{
	...
	uint32_t maxadd = m_Config->m_DynamicLatencyMaxToAdd;
	...
	for (uint32_t i=0; i<m_Players.size(); i++)
	{
		...
		{
			ping = m_Players[i]->GetPing(m_Config->m_LCPings);
	...
	if (highestping + m_Config->m_DynamicLatencyAddedToPing < m_Config->m_Latency)
		m_DynamicLatency = highestping + m_Config->m_DynamicLatencyAddedToPing;
	else
		m_DynamicLatency = m_Config->m_Latency + maxadd/2;

// if there is a lobby, increase latency to max
	if (m_Config->m_DynamicLatencyIncreasewhenLobby && m_GHost->m_CurrentGame)
	{
		m_DynamicLatency = m_Config->m_Latency + maxadd/3;

// reduce it to 2.2x highest ping if it is more than that.
		if (m_Config->m_DynamicLatency2xPingMax)
	...
	{
		m_DynamicLatency = m_Config->m_Latency + maxadd;
...
// modified to consider synclimit = 12
	...
	double latencydiff = m_Config->m_Latency - m_DynamicLatency;
	...
	if (m_DynamicLatency + tweaklatencyby > m_Config->m_Latency + maxadd)
		tweaklatencyby = m_Config->m_Latency + maxadd - m_DynamicLatency;
...
// don't go lower than 20/50 ms
	...
	percent = m_DynamicLatency * 100 / m_Config->m_Latency;
	percents = UTIL_ToString(percent, 0);

	int diff = m_Config->m_Latency - m_DynamicLatency;
	uint32_t absdiff = abs(diff);
	string op = string();
	if (m_DynamicLatency < m_Config->m_Latency)
		op = "-";
	else if (m_DynamicLatency > m_Config->m_Latency)
		op = "+";
	if (op.empty())
		...
		Console = "[GAME: " + m_GameName + "] Dynamic latency at "+percents+"% = "+UTIL_ToString(m_DynamicLatency)+" ms="+UTIL_ToString(m_Config->m_Latency)+op+UTIL_ToString(absdiff)+""+" sync="+UTIL_ToString(syncpercent)+"% - "+m_MaxSyncUser+"";
