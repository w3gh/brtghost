...
CAdminGame :: CAdminGame( CGHost *nGHost, CConfigData* nConfig, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nPassword ) : CBaseGame( nGHost, nConfig, nMap, nSaveGame, nHostPort, nGameState, nGameName, string( ), string( ), string( ) )
{
	m_Config = new CConfigData( *nConfig );
	m_Config->m_VirtualHostName = "|cFFC04040Admin";
	m_MuteLobby = true;
...
{
	SendChat( player, "brtGHost 1.7.1 Admin Game              http://www.brt.org.ua/" );
	SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
...
bool CAdminGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	if( player->GetLoggedIn( ) )
	{
		...
		******************/
		...
		else if( Command == "getgames" )
		{
			if( m_GHost->m_CurrentGame )
				SendChat( player, tr("lang_0026", "$DESCRIPTION$", m_GHost->m_CurrentGame->GetDescription( ), 
																			...
																			"$MAX$", UTIL_ToString( m_Config->m_MaxGames ) ) ); // GameIsInTheLobby
			else
				SendChat( player, tr("lang_0027", "$CURRENT$", UTIL_ToString( m_GHost->m_Games.size( ) ), 
																			"$MAX$", UTIL_ToString( m_Config->m_MaxGames ) ) ); // ThereIsNoGameInTheLobby
		}
				...
				try
				{
					...
					else
					{
						...
						else if( Matches == 1 )
						{
							...
							SendChat( player, tr("lang_0029", m_Config->mapcfgpath + File ) ); // LoadingConfigFile
							CConfig MapCFG;
		...
		{
			if (Payload.empty())
				SendChat(player, "rehostdelay is set to "+ UTIL_ToString(m_Config->m_AutoRehostDelay));
			else
			{
