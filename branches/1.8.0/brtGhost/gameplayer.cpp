		...
		for( vector<CBNET *> :: iterator i = m_Game->m_GHost->m_BNETs.begin( ); i != m_Game->m_GHost->m_BNETs.end( ); i++ )
		{
			if( (*i)->GetServer( ) == m_JoinedRealm )
			{
				if( m_Game->GetGameState( ) == GAME_PUBLIC )
				{
					...
					(*i)->QueueChatCommand( tr("lang_0041"), m_Name, true ); // SpoofCheckByReplying
			}
		}
...
//			m_Socket->PutBytes( m_Game->m_GHost->m_GPSProtocol->SEND_GPSS_ACK( m_TotalPacketsReceived ) );
			m_Socket->PutBytes( CGPSProtocol::SEND_GPSS_ACK( m_TotalPacketsReceived ) );
...
void CGamePlayer :: ProcessPackets( )
{
	...
	while( !m_Packets.empty( ) )
	{
			...
			case CGameProtocol :: W3GS_PONG_TO_HOST:
				...
				if( Pong != 1 )
				{
					...
					if( !m_DownloadStarted || ( m_DownloadFinished && GetTime( ) - m_FinishedDownloadingTime >= 5 ) )
					{
						...
						if( m_Game->m_Config->m_PingDuringDownloads || !m_Game->IsDownloading( ) )
						{
		...
		else if( Packet->GetPacketType( ) == GPS_HEADER_CONSTANT )
		{
			...
			{
				if( m_Game->m_Config->gproxy_enable )
				{
					m_GProxy = true;
...
//					m_Socket->PutBytes( m_Game->m_GHost->m_GPSProtocol->SEND_GPSS_INIT( m_Game->m_Config->port_reconnect, m_PID, m_GProxyReconnectKey, m_Game->GetGProxyEmptyActions( ) ) );
					m_Socket->PutBytes( CGPSProtocol::SEND_GPSS_INIT( m_Game->m_Config->port_reconnect, m_PID, m_GProxyReconnectKey, m_Game->GetGProxyEmptyActions( ) ) );
...
//	m_Socket->PutBytes( m_Game->m_GHost->m_GPSProtocol->SEND_GPSS_RECONNECT( m_TotalPacketsReceived ) );
	m_Socket->PutBytes( CGPSProtocol::SEND_GPSS_RECONNECT( m_TotalPacketsReceived ) );
	...
	m_GProxyDisconnectNoticeSent = false;
	m_Game->SendAllChat( tr("lang_0220", m_Name ) ); // PlayerReconnectedWithGProxy
}
