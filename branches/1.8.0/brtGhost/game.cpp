...
CGame :: CGame( CGHost *nGHost, CConfigData* nConfig, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nOwnerName, string nCreatorName, string nCreatorServer ) : CBaseGame( nGHost, nConfig, nMap, nSaveGame, nHostPort, nGameState, nGameName, nOwnerName, nCreatorName, nCreatorServer )
{
	m_DBBanLast = NULL;
...
CGame :: ~CGame( )
{
	...
	for( vector<CDBGamePlayer *> :: iterator i = m_DBGamePlayers.begin( ); i != m_DBGamePlayers.end( ); i++ ) {
		if (IsAutoBanned((*i)->GetName()))
		{
			...
			if ( endtime > timehasleft + m_Config->m_AutoBanGameEndMins * 60 )
			{
...
//								MILLISLEEP(20);
						...
						if(WarnCount >= m_Config->m_BanTheWarnedPlayerQuota)
						{
							...
							{
								m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedBanAdd( Match->GetServer( ), Match->GetName( ), Match->GetIP( ), m_GameName, "Autowarn", tr("lang_1125"), m_Config->m_BanTimeOfWarnedPlayer, 0 ));
							...
							else
							{
								...
								{
									m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedBanAdd( (*i)->GetServer( ), Match->GetName( ), Match->GetIP( ), m_GameName, "Autowarn", tr("lang_1125"), m_Config->m_BanTimeOfWarnedPlayer, 0 ));
									m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedWarnUpdate( Match->GetName( ), 3, 2));
									m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedWarnUpdate( Match->GetName( ), 1, 3));
								}
							}
...
//					m_GHost->m_Callables.push_back( m_GHost->m_DB->ThreadedWarnForget( (*i)->GetName( ), m_GHost->m_GameNumToForgetAWarn ));
			...
			if( m_Stats )
			{
				...
				if (m_Config->m_UpdateDotaScoreAfterGame)
				{
...
//						m_GHost->m_CalculatingScores = true;
				...
				{
					if (m_Config->m_UpdateDotaEloAfterGame)
						if (m_GHost->DBType == "mysql")
						{
...
//		CONSOLE_Print( "[GAME: " + m_GameName + "] checking score for "+ m_ShowScoreOf );
		if (!m_GHost->m_CalculatingScores)
		m_PairedDPSChecks.push_back( PairedDPSCheck( "%", m_GHost->m_DB->ThreadedDotAPlayerSummaryCheck( m_ShowScoreOf, m_Config->m_ScoreFormula, UTIL_ToString(m_Config->m_ScoreMinGames), string() ) ) );
	...
	{
		if (!m_GHost->m_CalculatingScores)
			m_PairedDPSChecks.push_back( PairedDPSCheck( "n", m_GHost->m_DB->ThreadedDotAPlayerSummaryCheck( m_PlayerScoreCheck, m_Config->m_ScoreFormula, UTIL_ToString(m_Config->m_ScoreMinGames), string() ) ) );
	...
	if (m_Config->m_AddCreatorAsFriendOnHost && GetTime() > m_CreationTime + 15)
	{
		for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
...
void CGame :: EventPlayerLeft( CGamePlayer *player, uint32_t reason  )
{
	...
	for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
	{
		...
		if ( m_Config->m_SafelistedBanImmunity && (*j)->IsSafe(player->GetName( ) ))
		{
			isAdmin = true;
			break;
		}
	...
	if (m_Config->m_AutoBan && !isAdmin) {
		...
		if (m_GameLoaded && !m_GameEnded) {
			...
			if (m_EvenPlayeredTeams) {
				...
				if (m_TeamDiff > m_Config->m_AutoBanTeamDiffMax && m_Config->m_AutoBanTeamDiffMax > 0) {	m_BanOn = false; }
			}
			...
			if (m_Config->m_AutoBanFirstXLeavers > 0 && m_PlayersLeft < m_Config->m_AutoBanFirstXLeavers) { m_BanOn = true; }
			...
			if (m_Config->m_AutoBanTimer > 0) {
				...
				if (iTime > m_Config->m_AutoBanTimer) { m_BanOn = false; }
			}
		} else if (m_GameLoading && m_Config->m_AutoBanGameLoading) {
			// If game is loading and player is not a admin ban.
			m_BanOn = true;
		} else if ( m_CountDownStarted && !m_GameLoading && !m_GameLoaded && m_Config->m_AutoBanCountDown ) {
			// If game is loading and player is not a admin ban
			m_BanOn = true;
		}
		...
		if (m_BanOn) 
		{
			string timediff = UTIL_ToString(m_Config->m_AutoBanGameEndMins);
...
//		if( m_DoAutoWarns && GetNumPlayers() >= m_GetMapOnlyAutoWarnIfMoreThanXPlayers && ( m_GameLoaded ) && player->GetLeftReason( ).compare( m_GHost->m_Language->HasLeftVoluntarily( ) ) == 0 )
		...
		if(LeftEarly > 0)
		{
			...
			for( vector<CBNET *> :: iterator j = m_GHost->m_BNETs.begin( ); j != m_GHost->m_BNETs.end( ); j++ )
			{
				if( (m_Config->m_SafelistedBanImmunity && IsSafe( player->GetName())) || IsAdmin( player->GetName() ) || (*j)->IsRootAdmin( player->GetName() ) )
				{
					LeftEarly = 0;
					break;
				}
...
bool CGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload )
{
	...
	if (BluePlayer && m_Config->m_BlueIsOwner)
	{
		AdminCheck = true;
...
// upgrade local players to admins in case bot_localadmins = 1
	if ((m_Config->m_LanAdmins && player->IsLAN()) || (m_Config->m_LocalAdmins && player->GetExternalIPString()=="127.0.0.1" ))
	{
		AdminCheck = true;
		AdminAccess = m_GHost->CMDAccessAddOwner(AdminAccess);
	}
	...
	if ((m_Config->m_LanRootAdmins && player->IsLAN()) )
	{
		RootAdminCheck = true;
	}
...
//					system("update_dota_elo.exe");
			...
			else if( ( Command == "delban" || Command == "unban" || Command == "db" || Command == "ub" ) && !Payload.empty( ) )
			{
				...
				if (m_Config->m_AdminsLimitedUnban && !RootAdminCheck)
				...
				if (m_Config->m_AdminsCantUnbanRootadminBans && BannedByRoot && !RootAdminCheck)
					admin = player->GetName();
...
//					SendChat(player->GetPID(), "No match found" );
			...
			else if(!m_Config->m_ReplaceBanWithWarn && ( Command == "addban" || Command == "ban" || Command == "b" ) && !Payload.empty( ) && !m_GHost->m_BNETs.empty( ) )
			{
				...
				if( m_GameLoaded )
				{
					...
					else if( Matches == 1 )
					{
						...
						if (IsSafe(LastMatch->GetName()) && m_Config->m_SafelistedBanImmunity)
						{
...
//						SendAllChat( m_GHost->m_Language->UnableToBanNoMatchesFound( Victim ) );
					if( Matches <= 1 )
					{
						...
						if (IsSafe(BanPlayer) && m_Config->m_SafelistedBanImmunity)
						{
							SendChat( player->GetPID(), tr("lang_1068")); // "You can't ban a safelisted player!"
							return HideCommand;
						}
...
//						m_GHost->m_DB->BanAdd( LastMatch->GetJoinedRealm( ), LastMatch->GetName( ), LastMatch->GetExternalIPString( ), m_GameName, User, Reason );
			...
			else if( ( (Command == "addwarn" || Command == "warn" || Command == "w") || (m_Config->m_ReplaceBanWithWarn && (Command == "ban" || Command == "addban" || Command == "b") )) && !Payload.empty( ) && !m_GHost->m_BNETs.empty( ) )
			{
				if (!CMDCheck(CMD_ban, AdminAccess))
				{
					SendChat(player->GetPID(), tr("lang_0005"));
				...
				if( m_GameLoaded )
				{
					...
					else if( Matches == 1 )
					{
						...
						if ((IsSafe(LastMatch->GetName()) && m_Config->m_SafelistedBanImmunity) && !RootAdminCheck)
						{
			...
			else if( Command == "autosave" )
			{
				if( Payload == "on" )
				{
					...
					m_Config->m_AutoSave = true;
				}
				else if( Payload == "off" )
				{
					SendAllChat( tr("lang_0143") ); // AutoSaveDisabled
					m_Config->m_AutoSave = false;
				}
			}
			...
			{
				if( !m_Config->m_AutoWarnEarlyLeavers )
				{
			...
			//if ()
			else if(!m_Config->m_ReplaceBanWithWarn && ( Command == "banlast" || Command == "bl" || Command == "blast" ) && m_GameLoaded && !m_GHost->m_BNETs.empty( ) && m_DBBanLast )
			{
				if (!CMDCheck(CMD_ban, AdminAccess))
				{
					SendChat(player->GetPID(), tr("lang_0005"));
				...
				if (IsSafe(m_DBBanLast->GetName( )) && m_Config->m_SafelistedBanImmunity)
				{
					SendChat( player->GetPID(), tr("lang_1068") ); // You can't ban a safelisted player!
					return HideCommand;
				}
			...
			else if( ( ((Command == "warnlast" || Command=="wlast" ||Command == "wl") || (m_Config->m_ReplaceBanWithWarn && (Command == "banlast" || Command == "blast" || Command == "bl"))) ) && m_GameLoaded && !m_GHost->m_BNETs.empty( ) && m_DBBanLast )
			{
				...
				if ((IsSafe(m_DBBanLast->GetName( )) && m_Config->m_SafelistedBanImmunity) && !RootAdminCheck)
				{
					SendChat( player->GetPID(), tr("lang_1070")); // "You can't warn a safelisted player!"
					return HideCommand;
				}
			...
			else if( Command == "check" )
			{
				if( !Payload.empty( ) )
				{
					...
					if( Matches == 0 )
                        ...
                        SendAllChat( tr("lang_0122", "$VICTIM$", LastMatch->GetName( ),
                                                                                "$PING$",  LastMatch->GetNumPings( ) > 0 ? UTIL_ToString( LastMatch->GetPing( m_Config->m_LCPings ) ) + tr("lang_1017") /* ms */ : tr("lang_1018") /* N/A */,
				...
				else
				{
                    ...
                    SendAllChat( tr("lang_0122", "$VICTIM$", User,
                                                                                "$PING$",  player->GetNumPings( ) > 0 ? UTIL_ToString( player->GetPing( m_Config->m_LCPings ) ) + tr("lang_1017") /* ms */ : tr("lang_1018") /* N/A */,
			...
			else if( Command == "clearhcl" && !m_CountDownStarted )
			{
				...
				if ( BluePlayer && AdminAccess == m_Config->m_OwnerAccess && !m_Config->m_BlueCanHCL )
					return HideCommand;
			...
			else if( ( Command == "dltspeed" ) && !Payload.empty( ) )
			{
				...
				if (Payload == "max")
					m_Config->m_totaldownloadspeed = 0;

				SendAllChat( tr("lang_1164", "$SPEED$", Payload, "$PRE_SPEED", UTIL_ToString(m_Config->m_totaldownloadspeed)) ); // "Total download speed = "+ Payload+" KB/s, previously "+UTIL_ToString(tspeed)+" KB/s"
			}
			...
			else if( ( Command == "dlspeed" ) && !Payload.empty( ) )
			{
				...
				if (Payload=="max")
					m_Config->m_clientdownloadspeed = 0;

				SendAllChat( tr("lang_1165", "$SPEED$", Payload, "$PRE_SPEED", UTIL_ToString(m_Config->m_clientdownloadspeed)) ); // "Maximum player download speed = "+ Payload+" KB/s, previously "+UTIL_ToString(tspeed)+" KB/s"
			}
			...
			else if( ( Command == "dlmax" ) && !Payload.empty( ) )
			{
				...
				if (Payload=="max")
					m_Config->m_maxdownloaders = 0;

				SendAllChat( tr("lang_1166", "$COUNT$", Payload, "$PRE_COUNT", UTIL_ToString(m_Config->m_maxdownloaders))); // "Maximum concurrent downloads = "+ Payload+", previously "+UTIL_ToString(t)
			}
						...
						if (!m_EndRequested)
						{
							...
							for( vector<CGamePlayer *> :: iterator i = m_Players.begin( ); i != m_Players.end( ); i++ )
							{
								if (m_Slots[GetSIDFromPID((*i)->GetPID())].GetTeam()!=m_EndRequestedTeam)
									SendChat((*i)->GetPID(), tr("lang_1169", "$USER$", User, "$TRIGGER$", string(1, m_Config->m_CommandTrigger) )); // " User + wants to end the game, type "+m_Config->m_CommandTrigger+"end to accept"
								else
					...
					if (m_Config->m_EndReq2ndTeamAccept && !RootAdminCheck)
					{
						...
						if (m_GetMapNumTeams==2 && secondTeamPresent)
						{
							...
							if (!m_RequestedWinner)
							{
								...
								{
									if (m_Slots[GetSIDFromPID((*i)->GetPID())].GetTeam()!=m_EndRequestedTeam)
										SendChat((*i)->GetPID(), tr("lang_1173", "$USER$", User, "$WINNER$", winnerString, "$TRIGGER$", string(1, m_Config->m_CommandTrigger)) ); // wants to end the game with winner "+winnerString+", type "+m_Config->m_CommandTrigger+"end "+UTIL_ToString(m_RequestedWinner)+" to accept");
									else
...
//					P = m_GHost->UDPChatWhoIs((*i)->GetExternalIPString());
			...
			else if( Command == "l" )
			{
				...
				Ping = Player->GetPing( m_Config->m_LCPings );
...
//				else // Unable to mute, more than one match found
	...
	// nobody can kick a rootadmin
			...
			else if( Command == "latency" || Command == "dr")
			{
				...
				{
					if (m_Config->m_UseDynamicLatency)
						SendAllChat( tr( "lang_1193", "$LATENCY$", UTIL_ToString( m_Config->m_Latency ), "$DLATENCY$", UTIL_ToString( m_DynamicLatency ))); // "Latency set at [" + UTIL_ToString( m_Latency ) + "], dynamic latency is ["+ UTIL_ToString( m_DynamicLatency )+"]"
					else
						SendAllChat( tr( "lang_0097", UTIL_ToString( m_Config->m_Latency ) ) ); // LatencyIs
				}
				else
				{
					...
					{
						m_Config->m_Latency = minLatency;
						SendAllChat( tr( "lang_0057", UTIL_ToString(minLatency) ) ); // SettingLatencyToMinimum
					}
					else if( m_Config->m_Latency >= 500 )
					{
						m_Config->m_Latency = 500;
						SendAllChat( tr( "lang_0058", "500" ) ); // SettingLatencyToMaximum
					}
					else
						SendAllChat( tr( "lang_0059", UTIL_ToString( m_Config->m_Latency ) ) ); // SettingLatencyTo
				}
			}
			...
			else if( Command == "messages" )
			{
				if( Payload == "on" )
				{
					...
					m_Config->m_LocalAdminMessages = true;
				}
				else if( Payload == "off" )
				{
					SendAllChat( tr("lang_0201") ); // LocalAdminMessagesDisabled
					m_Config->m_LocalAdminMessages = false;
				}
			}
			...
			else if( Command == "ping" || Command == "p" )
			{
				...
				if (!Payload.empty())
				{
					...
					else if( Matches == 1 )
					{
						...
						{
							Pings += UTIL_ToString( LastMatch->GetPing( m_Config->m_LCPings ) );
				...
				for( vector<CGamePlayer *> :: iterator i = SortedPlayers.begin( ); i != SortedPlayers.end( ); i++ )
				{
					...
					if( (*i)->GetNumPings( ) > 0 )
					{
						Pings += UTIL_ToString( (*i)->GetPing( m_Config->m_LCPings ) );

						if( !m_GameLoading && !m_GameLoaded && !(*i)->GetReserved( ) && KickPing > 0 && (*i)->GetPing( m_Config->m_LCPings ) > KickPing )
						{
							(*i)->SetDeleteMe( true );
							(*i)->SetLeftReason( tr("lang_1189", "$PING$", UTIL_ToString( (*i)->GetPing( m_Config->m_LCPings ) ), "$PINGMAX$", UTIL_ToString( KickPing ) )); // "was kicked for excessive ping " + UTIL_ToString( (*i)->GetPing( m_GHost->m_LCPings ) ) + " > " + UTIL_ToString( KickPing )
							(*i)->SetLeftCode( PLAYERLEAVE_LOBBY );
							OpenSlot( GetSIDFromPID( (*i)->GetPID( ) ), false );
							Kicked++;
						}
	...
	//				m_GHost->UDPChatSend("|rehost "+GameName);
			...
			else if( Command == "refresh" && !m_CountDownStarted )
			{
				if( Payload == "on" )
				{
					...
					m_Config->m_RefreshMessages = true;
				}
				else if( Payload == "off" )
				{
					SendAllChat( tr("lang_0091") ); // RefreshMessagesDisabled
					m_Config->m_RefreshMessages = false;
				}
			}
			...
			else if( Command == "sendlan" && !Payload.empty( ) && !m_CountDownStarted )
			{
				...
				else
				{
					...
					if( m_SaveGame )
					{
						...
						m_GHost->m_UDPSocket->SendTo( IP, Port, m_Protocol->SEND_W3GS_GAMEINFO( m_Config->tft, m_Config->m_LANWar3Version, UTIL_CreateByteArray( MapGameType, false ), m_Map->GetMapGameFlags( ), MapWidth, MapHeight, m_GameName, m_CreatorName, GetTime( ) - m_CreationTime, "Save\\Multiplayer\\" + m_SaveGame->GetFileNameNoPath( ), m_SaveGame->GetMagicNumber( ), 12, 12, m_HostPort, FixedHostCounter, m_EntryKey ) );
					}
					else
					{
						uint32_t MapGameType = MAPGAMETYPE_UNKNOWN0;

						m_GHost->m_UDPSocket->SendTo( IP, Port, m_Protocol->SEND_W3GS_GAMEINFO( m_Config->tft, m_Config->m_LANWar3Version, UTIL_CreateByteArray( MapGameType, false ), m_Map->GetMapGameFlags( ), m_Map->GetMapWidth( ), m_Map->GetMapHeight( ), m_GameName, m_CreatorName, GetTime( ) - m_CreationTime, m_Map->GetMapPath( ), m_Map->GetMapCRC( ), 12, 12, m_HostPort, FixedHostCounter, m_EntryKey ) );
					}
				}
			}
...
/*
			...
			{
				if( Payload.empty( ) )
					SendAllChat( tr("lang_0098", UTIL_ToString( m_Config->m_SyncLimit ) ) ); // SyncLimitIs
				else
				{
					m_Config->m_SyncLimit = UTIL_ToUInt32( Payload );

					if( m_Config->m_SyncLimit <= 10 )
					{
						m_Config->m_SyncLimit = 10;
						SendAllChat( tr("lang_0099", "10" ) ); // SettingSyncLimitToMinimum
					}
					else if( m_Config->m_SyncLimit >= 10000 )
					{
						m_Config->m_SyncLimit = 10000;
						SendAllChat( tr("lang_0100", "10000" ) ); // SettingSyncLimitToMaximum
					}
					else
						SendAllChat( tr("lang_0101", UTIL_ToString( m_Config->m_SyncLimit ) ) ); //  SettingSyncLimitTo
				}
			}
			...
			{
				DeleteVirtualHost( );
				m_Config->m_VirtualHostName = Payload;
			}
			...
			else if( Command == "rmk" && !player->GetRmkVote( ) && m_GameLoaded)
			{
				...
				if( Votes >= VotesNeeded )
				{
					...
					SendAllChat( tr("lang_1054", "$USER$", User, "$VOTE$", UTIL_ToString( Votes ), "$MAXVOTE$", UTIL_ToString(VotesNeeded), "$TRIGGER$", string( 1, m_Config->m_CommandTrigger ) )); // SendAllChat( User+" voted for rmk [" + UTIL_ToString( Votes )+"/"+ UTIL_ToString(VotesNeeded)+"] "+string( 1, m_Config->m_CommandTrigger )+"rmk to accept");
			}
	...
	else
	{
		...
		if ( Command != "yes" && Command !="votekick" && Command != "rmk" && !m_Config->m_NonAdminCommands)
			return HideCommand;
	}
	...
	*********************/
	...
	//
		...
		SendChat( player,  player->GetNumPings( ) > 0 ? UTIL_ToString( player->GetPing( m_Config->m_LCPings ) ) + tr("lang_1017") : tr("lang_1018")); // "ms" : "N/A"
			...
			{ 
				if ( player->GetSID() != ( SID1 - 1 ) && SID1 < 11 && SID1 > 0 )
				{
									  ...
									  && GetTime() - Player2->GetSwapToTime() < 30)
					{
						...
						SendChat(Player2, tr("1506", "$PLAYER$", player->GetName(), "$COMMAND$", string( 1, m_Config->m_CommandTrigger ) + "swap " + UTIL_ToString( player->GetSID() + 1 ) ));
				}
			}
	...
	else if( Command == "statsdota" && GetTime( ) >= player->GetStatsDotASentTime( ) + 5 && !m_Config->m_NoStatsDota )
	{
		string StatsUser = User;
		...
		if( player->GetSpoofed( ) && ( AdminCheck || RootAdminCheck || IsOwner( User ) ) )
			m_PairedDPSChecks.push_back( PairedDPSCheck( string( ), m_GHost->m_DB->ThreadedDotAPlayerSummaryCheck( StatsUser, m_Config->m_ScoreFormula, UTIL_ToString(m_Config->m_ScoreMinGames), string() ) ) );
		else
			m_PairedDPSChecks.push_back( PairedDPSCheck( User, m_GHost->m_DB->ThreadedDotAPlayerSummaryCheck( StatsUser, m_Config->m_ScoreFormula, UTIL_ToString(m_Config->m_ScoreMinGames), string() ) ) );
...
//			SendChat( player, m_GHost->m_Language->VersionNotAdmin( m_GHost->m_Version ) );
	...
	else if( Command == "votekick" && m_Config->m_VoteKickAllowed && !Payload.empty( ) )
	{
			...
			else if( Matches == 1 )
			{
				...
				else
				{
					...
					SendAllChat( tr("lang_0157",
                                               "$VICTIM$", LastMatch->GetName( ),
                                               "$USER$", User,
                                               "$VOTESNEEDED$", UTIL_ToString( (uint32_t)ceil( ( GetNumHumanPlayers( ) - 1 ) * (float)m_Config->m_VoteKickPercentage / 100 ) - 1 ) ) ); // StartedVoteKick

					SendAllChat( tr("lang_0165", string( 1, m_Config->m_CommandTrigger ) ) );  // TypeYesToVote
				}
			}
			else
	...
	{
		player->SetKickVote( true );
		uint32_t VotesNeeded = (uint32_t)ceil( ( GetNumHumanPlayers( ) - 1 ) * (float)m_Config->m_VoteKickPercentage / 100 );
		uint32_t Votes = 0;
...
*/
		if ( m_Config->m_minFFtime && ((GetTime() - GetGameLoadedTime()) < m_Config->m_minFFtime) )
		{
			SendChat(player->GetPID(), tr("lang_1217", UTIL_ToString(m_Config->m_minFFtime)));
			return HideCommand;
		}
		...
		SendChat(player->GetPID(), tr("lang_1210", "$TRIGGER$", string(1, m_Config->m_CommandTrigger)));
	}
	...
	else if( Command == "rmk" && !player->GetRmkVote( ) && m_GameLoaded)
	{
		...
		if( Votes >= VotesNeeded )
		{
			...
			SendAllChat( User+" voted for rmk [" + UTIL_ToString( Votes )+"/"+ UTIL_ToString(VotesNeeded)+"] "+string( 1, m_Config->m_CommandTrigger )+"rmk to accept");
	}
	...
	else if( (Command == "sd" || Command == "sdi" || Command == "sdpub" || Command == "sdpriv") && GetTime( ) >= player->GetStatsDotASentTime( ) + 5 )
	{
		...
		if (!nonadmin)
			m_PairedDPSChecks.push_back( PairedDPSCheck( "%", m_GHost->m_DB->ThreadedDotAPlayerSummaryCheck( StatsUser, m_Config->m_ScoreFormula, UTIL_ToString(m_Config->m_ScoreMinGames), GameState ) ) );
		else
			m_PairedDPSChecks.push_back( PairedDPSCheck( "%"+User, m_GHost->m_DB->ThreadedDotAPlayerSummaryCheck( StatsUser, m_Config->m_ScoreFormula, UTIL_ToString(m_Config->m_ScoreMinGames), GameState ) ) );
...
void CGame :: WarnPlayer( CDBBan *LastMatch, string Reason, string User)
{
	...
	if( WarnCount >= m_Config->m_BanTheWarnedPlayerQuota)
	{
	    if (m_Config->m_BanTimeOfWarnedPlayer)
            SendAllChat( tr("lang_0509", "$VICTIM$", LastMatch->GetName( ), "$BANDAYTIME$", UTIL_ToString( m_Config->m_BanTimeOfWarnedPlayer )));
        else
            SendAllChat( tr("lang_0510", LastMatch->GetName( ))); // UserReachedWarnQuota

		if (!isAdmin)
			m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetServer( ), LastMatch->GetName( ), LastMatch->GetIP( ), m_GameName, User, "Reached the warn quota", m_Config->m_BanTimeOfWarnedPlayer, 0 ));
		else
		{
			...
			for( vector<CBNET *> :: iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
			{
				if( (*i)->GetServer( ) == GetCreatorServer( ) )
				{
					...
					if(WarnCount >= m_Config->m_BanTheWarnedPlayerQuota)
					{
						...
						sBReason += UTIL_ToString(m_Config->m_BanTimeOfWarnedPlayer);
...
void CGame :: WarnPlayer( string Victim, string Reason, string User)
{
	...
	if( Matches <= 1 )
	{
		...
		if ((IsSafe(BanPlayer) && m_Config->m_SafelistedBanImmunity) && !RootAdminCheck)
		{
...
*/
        ...
        if (m_Config->m_BanTimeOfWarnedPlayer)
            SendAllChat( tr("lang_0509", "$VICTIM$", BanPlayer, "$BANDAYTIME$", UTIL_ToString( m_Config->m_BanTimeOfWarnedPlayer )));
        else
			...
			if (Matches==1)
				m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( LastMatch->GetSpoofedRealm( ), BanPlayer, LastMatch->GetExternalIPString( ), m_GameName, User, "Reached the warn quota", m_Config->m_BanTimeOfWarnedPlayer, 0 ));
			else
				m_GHost->m_Callables.push_back(m_GHost->m_DB->ThreadedBanAdd( m_Server, BanPlayer, string(), m_GameName, User, "Reached the warn quota", m_Config->m_BanTimeOfWarnedPlayer, 0 ));
