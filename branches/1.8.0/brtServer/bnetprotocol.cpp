...
vector<CIncomingGameHost *> CBNETProtocol :: RECEIVE_SID_GETADVLISTEX( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 8 )
	{
		...
		while( GamesFound > 0 )
		{
			...
			i += 4;
			BYTEARRAY GameName = UTIL_ExtractCStringBytes( data, i );
			i += GameName.size( ) + 1;

			if( data.size( ) < i + 1 )
				break;

			BYTEARRAY GamePassword = UTIL_ExtractCStringBytes( data, i );
			...
			i += 8;
			BYTEARRAY StatString = UTIL_ExtractCStringBytes( data, i );
			i += StatString.size( ) + 1;
...
bool CBNETProtocol :: RECEIVE_SID_ENTERCHAT( BYTEARRAY data )
{
	...
	{
		m_UniqueName = UTIL_ExtractCStringBytes( data, 4 );
		return true;
	}
...
CIncomingChatEvent *CBNETProtocol :: RECEIVE_SID_CHATEVENT( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 29 )
	{
		...
		BYTEARRAY User = UTIL_ExtractCStringBytes( data, 28 );
		BYTEARRAY Message = UTIL_ExtractCStringBytes( data, User.size( ) + 29 );
...
bool CBNETProtocol :: RECEIVE_SID_AUTH_INFO( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 25 )
	{
		...
		m_IX86VerFileName = UTIL_ExtractCStringBytes( data, 24 );
		m_ValueStringFormula = UTIL_ExtractCStringBytes( data, m_IX86VerFileName.size( ) + 25 );
		return true;
	}
...
bool CBNETProtocol :: RECEIVE_SID_AUTH_CHECK( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 9 )
	{
		...
		m_KeyStateDescription = UTIL_ExtractCStringBytes( data, 8 );
...
vector<CIncomingFriendList *> CBNETProtocol :: RECEIVE_SID_FRIENDSLIST( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 5 )
	{
		...
		while( Total > 0 )
		{
			...
			BYTEARRAY Account = UTIL_ExtractCStringBytes( data, i );
			i += Account.size( ) + 1;
			...
			i += 6;
			BYTEARRAY Location = UTIL_ExtractCStringBytes( data, i );
			i += Location.size( ) + 1;
...
vector<CIncomingClanList *> CBNETProtocol :: RECEIVE_SID_CLANMEMBERLIST( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 9 )
	{
		...
		while( Total > 0 )
		{
			...
			BYTEARRAY Name = UTIL_ExtractCStringBytes( data, i );
			i += Name.size( ) + 1;
...
CIncomingClanList *CBNETProtocol :: RECEIVE_SID_CLANMEMBERSTATUSCHANGE( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 5 )
	{
		BYTEARRAY Name = UTIL_ExtractCStringBytes( data, 4 );
		...
		if( data.size( ) >= Name.size( ) + 7 )
		{
			...
			BYTEARRAY Location = UTIL_ExtractCStringBytes( data, Name.size( ) + 7 );
...
CIncomingGameHost :: CIncomingGameHost( uint16_t nGameType, uint16_t nParameter, uint32_t nLanguageID, uint16_t nPort, BYTEARRAY &nIP, uint32_t nStatus, uint32_t nElapsedTime, string nGameName, unsigned char nSlotsTotal, uint32_t nHostCounter, BYTEARRAY &nStatString )
{
	...
	if( StatString.size( ) >= 14 )
	{
		...
		MapPath = UTIL_ExtractCStringBytes( StatString, 13 );
		i += MapPath.size( ) + 1;
		...
		{
			HostName = UTIL_ExtractCStringBytes( StatString, i );
