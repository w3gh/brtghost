...
CBNET :: CBNET( bool nTFT, string nWar3Path, string nServer, string nBNLSServer, uint16_t nBNLSPort, uint32_t nBNLSWardenCookie, string nCDKeyROC, string nCDKeyTFT, string nCountryAbbrev, string nCountry, string nUserName, string nUserPassword, string nFirstChannel, unsigned char nWar3Version, BYTEARRAY nEXEVersion, BYTEARRAY nEXEVersionHash, string nPasswordHashType, uint32_t nMaxMessageLength, CConfig* nConfig )
{
	...
	m_TFT = nTFT;
	m_Config = *nConfig;
	m_CommandTrigger = m_Config.command_trigger;
...
bool CBNET :: Update( void *fd, void *send_fd )
{
	...
	if( m_Socket->GetConnecting( ) )
	{
		...
		if( m_Socket->CheckConnect( ) )
		{
			...
			m_Socket->PutBytes( m_Protocol->SEND_SID_AUTH_INFO( m_War3Version, m_TFT, m_CountryAbbrev, m_Country ) );
			m_Socket->DoSend( (fd_set *)send_fd );
			m_LastNullTime = GetTime( );
			m_LastOutPacketTicks = GetTicks( );
...
//				break;
			...
			case CBNETProtocol :: SID_AUTH_INFO:
				...
				{
					if( m_BNCSUtil->HELP_SID_AUTH_CHECK( m_TFT, m_War3Path, m_CDKeyROC, m_CDKeyTFT, m_Protocol->GetValueStringFormulaString( ), m_Protocol->GetIX86VerFileNameString( ), m_Protocol->GetClientToken( ), m_Protocol->GetServerToken( ) ) )
					{
						...
						m_Socket->PutBytes( m_Protocol->SEND_SID_AUTH_CHECK( m_TFT, m_Protocol->GetClientToken( ), m_BNCSUtil->GetEXEVersion( ), m_BNCSUtil->GetEXEVersionHash( ), m_BNCSUtil->GetKeyInfoROC( ), m_BNCSUtil->GetKeyInfoTFT( ), m_BNCSUtil->GetEXEInfo( ), "brtServer" ) );
	...
	else if( Event == CBNETProtocol :: EID_WHISPER )
	{
		...
		if( !Message.empty( ) && Message[0] == m_CommandTrigger )
		{
			...
			if ( Command == "version" && Payload.empty() )
				QueueChatCommand( "brtServer version " + m_Config.client_lastversion, User, true);
			else
			m_chatCommands.push( CChatCommand(User, m_Server, Command, Payload) );
		}
