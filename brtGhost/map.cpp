...
//	m_CRC->Initialize( );
	...
	if ( m_Config->m_LANWar3Version == 23)
	{
...
void CMap :: Load( CConfig *CFG, string nCFGFile )
{
	...
	if( !m_MapLocalPath.empty( ) )
		m_MapData = UTIL_FileRead( m_Config->mappath + m_MapLocalPath );

	// load the map MPQ

	string MapMPQFileName = m_Config->mappath + m_MapLocalPath;
	HANDLE MapMPQ;
	...
	if( !m_MapData.empty( ) )
	{
		...
		string CommonJ = UTIL_FileRead( m_Config->mapcfgpath + "common.j" );

		if( CommonJ.empty( ) )
			CONSOLE_Print( "[MAP] unable to calculate map_crc/sha1 - unable to read file [" + m_Config->mapcfgpath + "common.j]" );
		else
		{
			string BlizzardJ = UTIL_FileRead( m_Config->mapcfgpath + "blizzard.j" );

			if( BlizzardJ.empty( ) )
				CONSOLE_Print( "[MAP] unable to calculate map_crc/sha1 - unable to read file [" + m_Config->mapcfgpath + "blizzard.j]" );
			else
			{
				uint32_t Val = 0;
					...
					if( SFileOpenFileEx( MapMPQ, "Scripts\\common.j", 0, &SubFile ) )
					{
						...
						if( FileLength > 0 && FileLength != 0xFFFFFFFF )
						{
							...
							if( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead ) )
							{
								...
								m_SHA->Update( (unsigned char *)SubFileData, BytesRead );
							}
				...
				if( !OverrodeCommonJ )
				{
					...
					m_SHA->Update( (unsigned char *)CommonJ.c_str( ), CommonJ.size( ) );
				}
				...
				if( !OverrodeBlizzardJ )
				{
					...
					m_SHA->Update( (unsigned char *)BlizzardJ.c_str( ), BlizzardJ.size( ) );
				}

				Val = ROTL( Val, 3 );
				Val = ROTL( Val ^ 0x03F1379E, 3 );
				m_SHA->Update( (unsigned char *)"\x9E\x37\xF1\x03", 4 );
	...
	if (m_LogAll)
	{
		...
		string File = m_Config->mapcfgpath + fmap + ".cfg";
		ofstream tmpcfg;
