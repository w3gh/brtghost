...
{
	string name;
	string ip; // ExternalIPstring() from CGamePlayer
};
...
protected:
	...
	string m_Password;
	vector<CachePlayer> cache_admin_players;
...
public:
	CAdminGame( CGHost *nGHost, CConfigData* nConfig, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nPassword );
	virtual ~CAdminGame( );
