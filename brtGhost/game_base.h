...
//	uint32_t m_SyncLimit;							// the maximum number of packets a player can fall out of sync before starting the lag screen
	...
	uint32_t m_LastDBstatsUpdateTime;               // GetTime when the last stats DB was updated
...
//	bool m_NormalCountdown;
	...
	CBaseGame( CGHost *nGHost, CConfigData* nConfig, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, string nGameName, string nOwnerName, string nCreatorName, string nCreatorServer );
	virtual ~CBaseGame( );
